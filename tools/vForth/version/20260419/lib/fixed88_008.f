...
: FP-SQRT ( fp -- sqrt-fp )
    DUP 0> IF                   \ negative or zero?
        DUP 2/ DUP 2 RSHIFT +   \ Initial guess: roughly x/2 + x/8
        >FP                     \ convert back to fixed point
        6 0 DO                  \ max 6 iterations
            2DUP FP/            \ x / xn
            OVER FP+ 2/         \ (xn + x/xn)/2  ? new guess
            2DUP = IF           \ if converged
                LEAVE           \ exit loop early
            THEN
            NIP                 \ drop old xn, keep new one
        LOOP
        NIP                     \ drop original x, leave result
    ELSE
        DROP 0                  \ return 0
    THEN
;
