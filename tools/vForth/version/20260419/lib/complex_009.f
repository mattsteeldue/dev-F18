...
: C*   ( mag1 ang1 mag2 ang2 -- mag3 ang3 )
  \ Multiplication: (r1,µ1) * (r2,µ2) = (r1*r2, µ1+µ2)
  ROT FP+           \ Add angles: µ1 + µ2
  NORMALIZE-ANGLE   \ Keep in [0,2Pi)
  -ROT FP*          \ Multiply magnitudes: r1 * r2
  SWAP ;

: C/   ( mag1 ang1 mag2 ang2 -- mag3 ang3 )
  \ Division: (r1,µ1) / (r2,µ2) = (r1/r2, µ1-µ2)
  NEGATE ROT FP+    \ Subtract angles:  -µ2 + µ1 
  NORMALIZE-ANGLE
  -ROT FP/          \ Divide magnitudes: r1 / r2
  SWAP ;

: C-CONJ   ( mag ang -- mag -ang )
  \ Complex conjugate: (r,µ) --> (r,-µ)
...
: C-POW-N   ( mag ang n -- mag^n ang*n )
  \ Integer power: (r,µ)^n = (r^n,nµ)
  TUCK                    \ r n µ n
  FP* NORMALIZE-ANGLE     \ Multiply angle by n
  -ROT FP-1.0 SWAP        \ µ*n r 1.0 n 
  0 ?DO                   \ µ*n r 1.0
    OVER FP*              
  LOOP                    \ µ*n r r^n  
  NIP SWAP ;
...
: C-SQRT   ( mag ang -- sqrt-mag ang/2 )
  \ Square root: sqrt(r,µ) = (sqrt r,µ/2)
  2/ 
  SWAP FP-SQRT SWAP ;

: C-ROOT-N   ( mag ang n -- root-mag ang/n )
  \ Nth root: sqrt(r,µ) = (ªsqrt r,µ/n)
  >R 
  R@ /              \ Divide angle by n
  SWAP 
...
: C>XY   ( mag ang-rad -- x y )
  \ Convert polar to cartesian: x = r*cos(µ), y = r*sin(µ)
  2DUP              \ r µ r µ
  FCOS              \ r µ r cosµ
  FP*               \ r µ rcosµ
  -ROT              \ rcosµ r µ 
