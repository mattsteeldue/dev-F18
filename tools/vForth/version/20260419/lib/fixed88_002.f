...
#4000  ,    \ three
#40000 ,    \ four
#40    ,    \ four when double-integer exceed 65535
#400   ,    \ five digits
#4000  ,    \ six digits
#40000 ,    \ seven digits
...
: D>F ( d -- f )
    ...
    DPL @ IF
        DUP IF                  \ double-integer larger than 65535
            DPL @ CELLS 
            SCALE + @           \ get corresponding scale divisor
            >R                  \ and save for later
            500 UM/MOD NIP 0    \ scale down
            2DUP D+             \ multiply 2 times for better percision.
        ELSE      
            DPL @ 1- CELLS 
            SCALE + @           \ get corresponding scale divisor
            >R                  \ and save for later
            2DUP D+
            2DUP D+             \ multiply 4 times for better precision
            OVER IF
                1 0 D+          \ add for better rounding
            THEN
        THEN
        R@                      \ fetch divisor
...
2.0 
2DUP     D>F  CONSTANT F2.0    
2DUP 2/  D>F  CONSTANT F1.0      \ 1.0
2DUP 2/  D>F  CONSTANT F0.5      \ 0.5 in 8.8
2DUP 2/  D>F  CONSTANT F0.25   
     2/  D>F  CONSTANT F0.125  
