...
- Filename (ASCII) left justified, space
filled
- Type (ASCII) left justified, space filledd
- Size in kilobytes (binary)
...
A(bits 6..0)=block size in sectors, 1 (0.5K) to 32 (16K)
...
HL=filespec, terminated with $ff
...
HL=address of supported filetypes buffer, laid out as follows:
...
return)
$08, BROWSERCAPS2_SORT
- directory sorting allowed (uses IDE_BANK)
...
defm “XYZ:”
; match this filetype and return to caller with it
defb 12
; length of second entry
defm “X:.hexdump |”
; match this filetype and execute .hexdump on it
defb 3
; length of third entry
defm “Z?:”
; matches .z3, .z4, .z5 etc
defb 3
; length of fourth entry
defm “Z*:”
; matches .z, .zip etc
defb $ff
; table terminator
...
HL=address of tokenized BASIC command line, terminated with $0d
...
required window has been made current via ROM 3 / $1601
...
B=0 for standard variable, B=1 for array
...
Fc=0, real-time-clock module not present
...
A=0, close all files, unmap all drives and swap partitions
...
A=0, query current mode information
A=1, change mode to:
B=layer (0,1,2)
...
ld
a,14 ; clear window control code
rst
$10
For all layers/modes except for layer 0, you can also use the other window
control codes in this way (for example to change character width, enable doublewidth/height etc). It is good practice to restore any settings that you change
...
B=0, tokenise BASIC line (plain text -> tokenised)
...
; executes C:/DOT/mydot
; executes /mydot on current drive
; executes mydot from current directory on current drive
...
address usually points within a BASIC statement, but may also be a systemsupplied null-terminated line). A ':' character within double-quotes does *not*
...
requests. The driver's API is accessible from the M_DRVAPI hook (in the esxDOScompatible API), the IDE_DRIVER call (in the +3DOS-compatible API) and the
...
different international keyboard layouts, or perhaps to add support for a multikeystroke buffer.
...
output or pointer manipulation through their APIs which will allow other streamrelated BASIC commands to be used, eg:
PRINT #n;....
INPUT #n;....
INKEY$ #n
GOTO #n,value
...
(set current stream pointer)
(get current stream pointer to variable var)
(get current stream size/extent to variable var)
(wait for next input character from stream and store in var)
...
;
esx_eexist (18)
driver with same id already installed
;
esx_einuse (23)
no free driver slots available
;
esx_eloadingko (26)
...
esx_mode_open_creat
$08
open existing or create file
;
esx_mode_creat_noexist $04
create new file, error if exists
;
esx_mode_creat_trunc
$0c
create new file, delete existing
;
;
...
esx_seek_set
$00
set the fileposition to BCDE
;
esx_seek_fwd
$01
add BCDE to the fileposition
;
esx_seek_bwd
$02
subtract BCDE from the fileposition
; Exit (success):
;
Fc=0
;
...
;
+2(1)
file attributes (MS-DOS format)
;
+3(2)
timestamp (MS-DOS format)
;
+5(2)
datestamp (MS-DOS format)
;
+7(4)
file size in bytes
; ***************************************************************************
...
;
$00 esx_mode_short_only
F_READDIR returns short 8.3 names
;
$10 esx_mode_lfn_only
F_READDIR returns LFNs only
;
...
wildcard
;
$40 esx_mode_use_header
F_READDIR additionally returns +3DOS header
;
$80 esx_mode_sf_enable
enable sort/filter mode in C
;
;
C=sort/filter mode (if enabled in access mode) – add together:
;
$80 esx_sf_exclude_files
F_READDIR doesn't return files
;
$40 esx_sf_exclude_dirs
F_READDIR doesn't return directories
;
$20 esx_sf_exclude_dots
F_READDIR doesn't return . or .. directories
;
$10 esx_sf_exclude_sys
F_READDIR doesn’t return system/hidden files
;
$08 esx_sf_sort_enable
...
sort by LFN
;
$01 esx_sf_sort_short
sort by short name
;
$02 esx_sf_sort_date
sort by date/time (LFN breaks ties)
;
$03 esx_sf_sort_size
sort by file size (LFN breaks ties)
;
$04 esx_sf_sort_reverse
...
; read access
; open existing files only

; Next registers
next_register_select
nxr_peripheral2
...
; get current peripheral2 value
; clear bit 3 (multiface enable)
...
a,'*'
; use default drive if none specified
ix,test_filename
b,esx_mode_read+esx_mode_open_exist
$08
f_open
c,exit_with_error
(filehandle),a
...
; address to stream data to
; size of data left to stream
; save in alternate registers
...
; address of buffer
de,filemap_size
; size of buffer (in 6-byte entries)
$08
disk_filemap
c,close_and_exit_with_error
...
; store card flags for later use
...
a
; not needed as no error, so carry=0
hl,de
; any entries in the buffer at all?
hl
a,$ff
; dummy error to indicate out of data
z,close_and_exit_with_error
...
; save buffer end address
; HL=address of next entry in buffer

; BCDE=card address
; IXDE=card address
; BC=number of 512-byte blocks
...
disk_strmstart
ix
; retrieve number of blocks to IX
c,drop2_close_and_exit_with_error
...
; prepare for 256-byte INIR

a,d
2
c,stream_partial_block

; at least 1 block to stream?
...
; A=protocol (0=SD/MMC, 1=IDE)
; The IDE protocol doesn't need
...
; wait for start of next block
; (a token is != $ff)
; the correct data token is $fe
...
ld
a,d
; check if any more bytes needed
or
e
jr
z,streaming_complete
dec
ld
or
jr
...
a,ixl
ixh
nz,stream_block_loop

exx

; decrement block count
; continue until all blocks streamed
; switch "streaming set" to alternates
...
; DE=current buffer address
; HL=ending buffer address
; not needed; carry=0 since no error
; any more entries left in buffer?
; if not, refill
; re-form ending address
; back for next entry in the buffer
...
a
jr
z,stream_final_bytes
inir

; at least 256 bytes left?
; read 256 bytes from the port
...
; read last few bytes from the port
...
defb
disk_strmend
; terminate the streaming operation
jr
...
; discard buffer addresses

; save error status

; restore error status

; BASIC requires H'L'=$2758 on return
; BC=error, for return to BASIC
; exit if there was an error
; use 65535 to indicate "no error"
...
set
7,a
; set bit 7 on DivMMC bank id to page
;
jp
$2000
; jump to switch banks and "return"
;
...
;
ld
a,divmmcbankid ; (to be patched by .INSTALL)
;
call
call_externmmc
;
...
jp
$2000
; switch back to driver and return
;
...
; increment stored border colour
...
; check if B>=$80
; on for standard API functions if so
...
;
; The shutdown ($81) call does NOT need to deallocate the RAM banks ; this will be done by the .UNINSTALL dot command.
;
...
jr
z,driver_init
; on for call $80, initialise
dec
a
jr
nz,channel_api
; if not $81, must be a channel API call
...
; always safe to uninstall this driver
...
; always safe to install this driver
...
; set zero flag if call $f7
; (return output status)
; exit if unsupported (<$f7)
; B=0..8
; on if not $f7 (output status)
...
; our device always ready for output
; clear carry to indicate success
...
; our device always ready for input
; clear carry to indicate success
...
; OPEN #n,"D>X,p1,p2"
; open with numbers: DE=p1, HL=p2 (zeros if not
provided)
;
...
; exit with error if already open
; signal "channel open"
; exit with carry reset (from AND above)
; and A=handle=1
...
; return if so
; otherwise discard return address
; and exit with error
...
; check D is our handle (does not return
; if invalid)

(chanopen_flag),a

; signal "channel closed"
; exit with carry reset (from XOR)
...
; HL=address within buffer
; store character

(output_ptr),a

; update pointer
; exit with carry reset (from AND)
...
; exit with A=$fe and carry set if so
...
; number of relocation entries (0..255)
...
; number of additional 8K DivMMC RAM banks
...
; number of additional 8K Spectrum RAM banks
; required (0..200)
...
defb
bnk_patches
; number of driver patches for this bank id
;
defw
bnk_size
; size of data to pre-load into bank (0..8192)
;
; (remaining space will be erased to zeroes)
;
defs
bnk_size
; data to pre-load into bank
;
defs
bnk_patches*2
; for each patch, a 2-byte offset (0..511) in
;
...
defs
800,$aa
; 800 bytes filled with $AA
defm
"This is the first allocated ZX bank"
defs
20,$55
; 20 bytes filled with $55
b0data_end:
; List of patches to be replaced with this bank's id
defw
bankid_zx0
...
defm
"This is the third allocated ZX bank"
b2data_end:
; List of patches to be replaced with this bank's id
defw
bankid_zx2
...
; "output character" call?
; on if so
; "return output status" call?
; on if so
; A=0, unsupported call id
; Fc=1, signals error
...
; clear carry to indicate success
; get signals from printer
; check BUSY signal
; exit with BC=$ffff if not busy
; exit with BC=$0000 if bust
...
jr
c,check_printer
; on if CAPS SHIFT not pressed
ld
a,$fe
; exit with A=$fe and carry set
scf
; so "End of file" reported
ret
check_printer:
...
; standard driver id for printer device.
...
; number of 8K DivMMC RAM banks needed
; number of 8K Spectrum RAM banks needed
...
; start with KSTATE0..3
; on if the set is free
; otherwise decrease its counter
; signal set is free after 5 calls
...
; on if valid key
; else read kempston port
; exit if $ff (no Kempston port)
; exit if $00
...
; next table address
; next port bit to carry
; until found a set bit
; exit if more than one set bit
; A=code
...
; start with KSTATE0..3
; on for repeat if matching key
; again with KSTATE4..7
; on for repeat of 2nd set's key
; on with new key if 2nd set is free
; exit if neither set is free
; store "main" key in KSTATE0/4
; initialise counter in KSTATE1/5
; initialise repeat delay in KSTATE2/6
; C=(MODE) system variable
; D=(FLAGS) system variable

k_decode
hl
(hl),a

; fully decode key with current shifts

(LAST_K),a
5,(iy+iy_FLAGS)

; update last key
; and signal a new key is available

; store decoded key in KSTATE 3/7
...
; reset the set's 5-call counter
; decrement the repeat delay
; exit if too early to repeat
; set delay for further repeats
; fetch the fully-decoded key
; and register it as a new press
...
; initial value for each row: $2f..$28
; D=E=no key
; C=port, B=row
...
; exit with zero reset if this is the
; third key to be detected
...
; find keycode value for each bit
; in turn
...
; next row's initial keycode
; next row port value
; back until all rows scanned

; copy any previous keycode to D
; E=keycode
; back if further keys in this row

; exit with zero set if 0 or 1 keys
; or if 2nd key of pair is CAPS SHIFT
; or if 2nd key of pair is SYM SHIFT
; SYM SHIFT could be 1st key of a pair
; so swap them
; and exit with zero set if so
; or zero reset for an invalid pair
...
; B=shift keycode ($ff/$27/$18)
; return with carry reset if the
; 1st key was CAPS or "no key"
; return with carry reset if
; only SYM shift pressed
; the main keytable
; A=decoded L-mode key
; carry set, valid key
...
; on for digits, SPACE, ENTER, EXTEND

m,k_decode_4
z,k_decode_2

; on for letters in K/L/C modes
; on for letters in E mode
...
z
add
a,token_sin-token_new

; D=shift key

; DRIVE+letters to TO..RESTORE
; unshifted letters to UDGs
; CAPS-shifted letters to NEW..COPY
; SYM-shifted letters to SIN..THEN
...
; on if neither shift pressed
; shifted E-mode table
...
; symbol code table
; back for SYM-SHIFTed letters
; on for K mode
; exit with upper-case letter in C mode
; or if CAPS SHIFT held
; otherwise convert to lower-case

; convert letter to keyword tokens
...
; no conversion for SPACE/ENTER/EXTEND
; on for K/L/C modes
; on for G mode
; extended mode table for digits
; use table if SYM SHIFT held
; on for '8' or '9'
; convert to "paper colour" $10..$17
; if no shift held
; convert to "ink colour" $18..$1f
; if CAPS SHIFT held
...
; convert to "bright code" $02..$03
; if no shift held
; convert to "flash code" $00..$01
; if CAPS SHIFT held
...
; control code keytable
; special case 9 (GRAPHICS mode)
; special case 0 (DELETE)
; SYM-shifted digits to 170..177
; unshifted digits to 128..135

; CAPS-shifted digits to 136..143
...
; use digits if unshifted

; use control code table if CAPS SHIFTed
; convert to: SPACE,!,",#,$,%,&,',(,)
; " needs to be replaced with @
; SPACE needs to be replaced with _
; SYM SHIFT+0 is _
; SYM SHIFT+2 is @
