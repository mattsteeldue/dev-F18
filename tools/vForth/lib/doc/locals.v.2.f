...
\ point and an ABORT in mid-declaration must not leave the locals in the
\ search order.
...
\ NOOP EXIT : the ABORT comes after (LOC-OPEN) has already revealed it.

: }  ( -- )
    1 ABORT" LOCALS: } without {"
;
...
: {  ( -- )
    ...
    BEGIN
        >IN @  BL WORD              ( in a )
        DUP C@ 0=  OVER 1+ C@ 0= OR ABORT" LOCALS: missing }"
        DUP C@ 1 = OVER 1+ C@ [CHAR] } = AND  0=
    WHILE                           ( in a )
        DROP  >IN !                 \ rewind: CONSTANT re-parses the name
        (LOC-MAKE)
        #LOCALS @ MAXLOCALS >       ABORT" LOCALS: bad count"
    REPEAT
    2DROP                           ( in a )
    #LOCALS @ 0=                    ABORT" LOCALS: bad count"
