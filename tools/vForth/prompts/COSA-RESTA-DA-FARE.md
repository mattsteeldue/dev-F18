...
1. Numerazione - parzialmente risolta: si e` confermato il precedente "append" (055?056?057, con lo slot DMA spostato da 057 a 058-dma.f). Resta da decidere per i gap rimanenti (tilemap, Q8.8, chomp-capstone): continuare ad appendere a 059+ o inserire in mezzo alla sequenza esistente?
2. Asset AFX - parzialmente risolta: lo scopo di tutorial/afx/ (centinaia di file .afx) e` ormai chiaro (materiale di tut. 050+054), ma resta aperto se spostarli fuori da tutorial/ (in work//assets/) per non gonfiare il repo, o lasciarli dove sono.
...
In sintesi: la parte "dot commands" e' chiusa da ieri; restano aperte le tre decisioni esplicite dell'autore (numerazione, asset AFX, riuso screen 880-881) piu` tre gap di contenuto (tilemap, Q8.8, ZAP) e due promozioni demo?tutorial (brot, Fedora) oltre al capstone chomp-chomp.