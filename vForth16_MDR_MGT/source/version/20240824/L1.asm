...
//
                ...
                Constant_Def CL         ,   "C/L"   ,  32
                Constant_Def BBUF       ,   "B/BUF" , 512
                Constant_Def BSCR       ,   "B/SCR" ,   1
...
//
// (does>)     --
                Colon_Def C_DOES, "(DOES>)", is_normal   
                dw      R_TO                    // r>
                dw      LATEST                  // latest
                dw      PFA                     // pfa 
...
// does>     --
                ...
                dw      COMPILE, C_DOES         // compile (does>)
                dw      LIT, $CD00, COMMA       // $CD00 ,
...
// Floating-Point-Operation
                New_Def FOP, "FOP", is_code, is_normal
                pop     hl     
                push    bc
                push    de
                ld      a, l
                ld      (Fop_Op),a 
                rst     $28
Fop_Op:         db $04  // this location is patched each time
                db $38  // end of calculation
                pop     de
                pop     bc
                next
...
// takes a double-number from stack and put to floating-pointer stack 
                New_Def TO_FPSTACK, ">W", is_code, is_normal
                exx
                pop     hl     
                pop     de     
...
//              ld      b, e    // maybe a better fit than c0h
                ...
                ld      a, h
                and     a
                jr      nz, To_FPStack_Endif
                    ld      h, d  // swap c and d 
                    ld      d, c
                    ld      c, h
To_FPStack_Endif:                // here disp, \ then,       
                call    $2ab6 
                exx
                next
...
// W>    ( -- d )
// takes a double-number from stack and put to floating-pointer stack 
                New_Def FROM_FPSTACK, "W>", is_code, is_normal
                exx
                call    $2bf1
                and     a
                jr      nz, From_FPStack_Endif
                    ld      h, d   // swap c and d
                    ld      d, c
                    ld      c, h
From_FPStack_Endif:                // here disp, \ then,       
                ld      h, a
                ld      l, e
