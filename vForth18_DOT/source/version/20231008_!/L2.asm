...
// prepare save-area address and hw register port
                ld      hl, Saved_MMU_Speed     // save-area (speed)

//  ______________________________________________________________________ 
//
// ask / read cpu-speed and MMU-status
...
//  ______________________________________________________________________ 
                ...
                ld      hl, $8000                                     !
                ld      de, $C000                                     !
                call    Backup_Restore_MMU      // dummy backup       !  

//  ______________________________________________________________________ 
// 
// ask current LAYER status 
...
//
// set LAYER 1,2
// this seems to change MMU3 from $0B to $11 ...
                exx
                ld      bc, $0102
                exx
                ld      de, $01D5
...
// do we really need to close this handle?
//              rst     8              
//              DEFB    $9B             ; f_close

// Better set current dir to /tools/vForth
                call    Set_Cur_Dir

//  ______________________________________________________________________ 
// 9.
// Reserve to Forth four main 16-bit registers
...
// reserve MMU pages
Restore_Reserve_MMU:
                ld      l, First_PAGE    // first page
                ld      h, Allocated_PAGES    // 8 HEAP, 4 MAIN, 2 BACKUP
Reserve_MMU_Loop:                
                ld      a, l            // pass page through a
                exx
                 // parameters:
Deallocate_MMU:                 
                 ld      hl, $0001      // L=1 allocate, =3 deallocate, H=0 normal 8k page
                 ld      e, a           // e' is bank-id
                exx
                push    hl
...
Set_forth_MMU:
                nextreg $53, MMU3_page   ;   MMU3  = 24576 $6000
                nextreg $54, MMU4_page   ;   MMU4  = 32768 $8000
                nextreg $55, MMU5_page   ;   MMU5  = 40960 $A000
                nextreg $56, MMU6_page   ;   MMU6  = 49152 $C000
                nextreg $57, MMU7_page   ;   MMU7  = 57344 $E000
                ret

//  ______________________________________________________________________ 
// Routine 
Backup_Restore_MMU:
                nextreg $54, $0B   ;   MMU4  = 32768 $8000
                nextreg $55, $11   ;   MMU5  = 40960 $A000
                nextreg $56, Backup_PAGE    ;   MMU6  = 49152 $C000
                nextreg $57, Backup_PAGE+1  ;   MMU7  = 57344 $E000
                ld      bc, $4000
                ldir
                ret
...
//  ______________________________________________________________________ 
// Input:  bc=$243B, a=reg, hl=array pointer
...
Go_Basic:
                ...
                // restore layer ide mode before restoring
                ld      a, (Saved_Layer)

                // prepare b'c' for  IDE_MODE ($01D5) call
                exx
                ld      b, a
                rrca
                rrca
                ...
                // restore backup page $0B and $11    !
                ld      hl, $C000                     !
                ld      de, $8000                     !
                call    Backup_Restore_MMU            ! 

                // restore speed and mmu pages
                ld      hl, Saved_MMU_Speed
                ld      bc , $243B
...
MMU_put_loop:                
                ...
                // deallocate MMU pages 
                ld      hl, Deallocate_MMU + 1
                ld      (hl), 3
                call    Restore_Reserve_MMU
